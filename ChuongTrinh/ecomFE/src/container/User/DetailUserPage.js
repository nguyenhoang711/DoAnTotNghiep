...
function DetailUserPage(props) {
    ...
    const [inputValues, setInputValues] = useState({
        firstName: '', lastName: '', address: '', phonenumber: '', genderId: '', dob: '', roleId: '', email: '', image: '', isActiveEmail: '', imageReview: '', imageSrc: '', isOpen: false
    });
    ...
    let handleSaveInfor = async () => {
        ...
        data.append('file', inputValues.imageSrc);
        data.append('id', id);
        data.append('firstName', inputValues.firstName);
        data.append('lastName', inputValues.lastName);
        data.append('address', inputValues.address);
        data.append('roleId', inputValues.roleId);
        data.append('phonenumber', inputValues.phonenumber);
        data.append('dob', inputValues.dob);
    ...
    let handleOnChangeImage = async (event) => {
        ...
        if (file.size > 2097152) {
            toast.error("Dung lượng file bé hơn 2mb")
        }
        else {
            let base64 = await CommonUtils.getBase64(file);
            let objectUrl = URL.createObjectURL(file);
            
            setInputValues({ ...inputValues, ["image"]: base64, ["imageReview"]: objectUrl, ["imageSrc"]: file})
