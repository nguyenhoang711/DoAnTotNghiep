...
const LoginWebPage = () => {
    ...
    let handleLogin = async () => {
        ...
        if (res && res.status === 200) {
            ...
            localStorage.setItem("userData", JSON.stringify(res.metadata.user))
            localStorage.setItem("token", JSON.stringify(res.metadata.accessToken))
            if (res.metadata.user.roleId === "R1" || res.metadata.user.roleId === "R4") {
    ...
    let handleSaveUser = async () => {
        ...
        if (res.isCheck === true) {
            ...
            const res = inputValues.lastName.split(' ', 1)
            setInputValues({
    ...
    let LoginWithSocial = async (re) => {
        ...
        if (res.isCheck === true) {
            ...
            let res = await createNewUser({
                ...
                phonenumber: re.user.providerData[0].phoneNumber,
                // avatar: re.user.providerData[0].photoURL,
                roleId: "R2",
                password: inputValues.password
            })
            if (res && res.errCode === 0) {
