...
let handleSendEmailConfirmOrder = (data) => {
    return new Promise(async (resolve, reject) => {
        try {
            if (!data.userId) {
                resolve({
                ...
                let user = await db.User.findOne({
                    where: { id: data.userId, statusId: 'S1' },
                    attributes: {
                        exclude: ['password']
                    },
                    raw: false
                ...
                if (user) {
                    await emailService.sendSimpleEmail({
                        firstName: user.firstName,
                        lastName: user.lastName,
                        redirectLink: `${process.env.URL_REACT}/user/order/${user.id}`,
                        email: user.email,
                        type: 'confirmorder'
                    })
                }
...
module.exports = {
    ...
    handleSendEmailConfirmOrder: handleSendEmailConfirmOrder,
